...
#include <preprocessorDefinitions.h>
  ...
  use aerosoldef, only: chemistryIndex, physicsIndex &
                        ...
                        , getCloudTracerName
  use condtend, only: N_COND_VAP, COND_VAP_ORG_SV, COND_VAP_ELVOC, COND_VAP_H2SO4 &
                      , condtend_sub
  use koagsub, only: coagtend, clcoag 
  use sox_cldaero_mod, only: sox_cldaero_init
  !smb+++ vbs
  use vbs_def, only: NPOA, NSOAv, NSOAP,oa_all_names, og_all_names,oa_all_names_p00
  !smb--- vbs
 ...
 ! Misc private data 
  ...
  integer :: ndx_h2so4, ndx_elvoc, ndx_soa_sv, ndx_dummy
  !smb+++vbs
  integer,dimension(NSOAv) :: ndxs_sog
  integer,dimension(NPOA)  :: ndxs_pog
  integer,dimension(NSOAv) :: ndxs_soa
  integer,dimension(NSOAP+1) :: ndxs_all_oa
  integer,dimension(NSOAP) :: ndxs_all_og
  !smb---vbs
  ! Namelist variables
...
contains
  ...
  subroutine aero_model_init( pbuf2d )
   ...
   call initopt_lw
    ...
    call cnst_get_ind ( "ELVOC", ndx_elvoc,abort=.true.)
    ndx_elvoc = chemistryIndex(ndx_elvoc)
    !call cnst_get_ind ( "SOA_SV", ndx_soa_sv, abort=.true.)
    ndx_soa_sv = chemistryIndex(ndx_soa_sv)

    !smb++ VBS
     do m=1,NSOAv
          WRITE(spec_name,'(A,I2.2)') 'SOG',m
          call cnst_get_ind ( spec_name, ndx_dummy,abort=.true.)
          ndxs_sog(m) = chemistryIndex(ndx_dummy)
     end do
      !smb++ VBS
      do m=1,NSOAv
          WRITE(spec_name,'(A,I2.2)') 'SOA',m
          call cnst_get_ind ( spec_name, ndx_dummy,abort=.true.)
          ndxs_soa(m) = chemistryIndex(ndx_dummy)
      end do
      do m=1,NPOA
          WRITE(spec_name,'(A,I2.2)') 'POG',m
          call cnst_get_ind ( spec_name, ndx_dummy,abort=.true.)
          ndxs_pog(m) = chemistryIndex(ndx_dummy)
      end do
      do m=1,NSOAP
  ...
  subroutine aero_model_gasaerexch( loffset, ncol, lchnk, troplev, delt, reaction_rates, &
    ...
    real(r8) :: del_elvoc_gasprod(ncol,pver)
    ...
    !do m = 1,NSOAP
    !    i = ndxs_all_og(m)
    !    cgas_vbs(1:ncol,:,m) =  mmr_tend_ncols(1:ncol,:,i)
    !end do
    !do m = 1,NSOAP
    !    i = ndxs_all_oa(m)
    !    caer_vbs(1:ncol,:, m) =  mmr_tend_ncols(1:ncol,:,i)
    !end do
    !do k=1,pver
    !    do i=1,ncol
    !
    !        call oracle_soap(caer_vbs(i,k,:),cgas_vbs(i,k,:),temp(i,k),i,k,csatT)
    !    end do
    !end do
...
!  Get mass mixing ratios at start of time step
   ...
   mmr_cond_vap_start_of_timestep(:ncol,:,COND_VAP_ELVOC) = mmr_tend_ncols(1:ncol,:,ndx_elvoc)
...
!
    ...
    del_elvoc_gasprod(1:ncol,:) = vmr(1:ncol,:,ndx_elvoc) - vmr0(1:ncol,:,ndx_elvoc)
...
!
   ...
   do k = 1,pver
      ...
      mmr_cond_vap_gasprod(:ncol,k,COND_VAP_ELVOC) = adv_mass(ndx_elvoc) * del_elvoc_gasprod(:ncol,k) / mbar(:ncol,k)/delt !cka
   ...
   mmr_tend_ncols(:ncol,:,ndx_elvoc)  = mmr_cond_vap_start_of_timestep(:ncol,:,COND_VAP_ELVOC)
