...
! 40 nm is assumed to have an efficient coagulation with other particles.
   ...
   use noresm_oracle, only : calc_loss
   implicit none
   ...
   logical                                          :: history_aerosol
    real(r8), dimension(pcols,pver)       :: soa_a1_before ! smb++ SOA_A1 before coagulation smb--
    real(r8), dimension(pcols,pver)                  :: dummy1
    real(r8), dimension(pcols,pver,gas_pcnst)                  :: dummy2
   totalLoss(:,:,:)=0.0_r8
   ...
   do k=1,pver
    ...
    !smb++ vbs: calculate SOA_A1 before loss rate
    soa_a1_before(:ncol,:) = q(:ncol,:,chemistryIndex(l_soa_a1))
    !smb--
   ...
   !smb++ vbs: reduce all vbs bins which sum up to SOA_A1 by same factor as SOA_A1
   dummy1 = soa_a1_before(:ncol,:)
   dummy2 = totalLoss(:ncol,:,:)
   dummy2 = q(:ncol,:,:)

   ! dummy2 = q
   WRITE(*,*) 'hey smb koagsub'
   !WRITE(*,*) soa_a1_before(:ncol,:)
   !WRITE(*,*) q(:ncol,:,:)
   !WRITE(*,*) totalLoss(:ncol,:,:)
   WRITE(*,*) ncol



   call calc_loss(soa_a1_before(:ncol,:), q(:ncol,:,:), ncol, totalLoss(:ncol,:,:))
   !smb--
...
!UPDATE THE TRACERS AND DO DIAGNOSTICS
   do iCoagulator = 1, numberOfCoagulatingModes
    ...
    !smb++ vbs: reduce all vbs bins which sum up to SOA_A1 by same factor as SOA_A1
    WRITE(*,*) 'hey smb koagsub cl '

  call calc_loss(soa_a1_before(:ncol,:), q(:ncol,:,:), ncol,cloudLoss(:ncol,:,:))
    !smb--
