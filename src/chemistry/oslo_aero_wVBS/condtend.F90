module condtend
   ...
   use vbs_def, only: NPOA, NSOAv, NSOAP
!soa
   ...
   integer, parameter :: COND_VAP_ELVOC = 2
...
!===============================================================================
   ...
   subroutine initializeCondensation()
      ...
      cond_vap_map(COND_VAP_ELVOC) = chemistryIndex(l_elvoc)
...
! Added input for h2so4 and soa nucleation: elvco_gasprod, soa_sv_gasprod, pblh,zm,qh20 (cka)
...
!nuctst3-
   real(r8), parameter :: lvocfrac=1.           !Fraction of organic oxidation products with low enough
                                                  !volatility to enter nucleation mode particles (1-24 nm)
   real(r8) :: elvoc_forNucleation(pcols,pver)  ![kg/kg] soa gas available for nucleation
...
!ak-
    ...
    do k=1,pver
      do i=1,ncol
         ...
         elvoc_forNucleation(i,k) = lvocfrac*intermediateConcentration(i,k,COND_VAP_ELVOC) !fraction of elvoc left that is assumend to have low enough
...
!ak-
   ...
   call aeronucl(lchnk,ncol,temperature, pmid, qh20, &
               intermediateConcentration(:,:,COND_VAP_H2SO4), elvoc_forNucleation, &
   ...
   do k=1,pver
      do i=1,ncol
         ...
         !First order loss rate (1/s) for nucleation
         firstOrderLossRateNucl(i,k,COND_VAP_ELVOC) = nuclSOA(i,k)/intermediateConcentration(i,k,COND_VAP_ELVOC)
         ...
         q(i,k,chemistryIndex(l_soa_na)) =  q(i,k,chemistryIndex(l_soa_na))       &
                     + gasLost(i,k,COND_VAP_ELVOC)*fracNucl(i,k,COND_VAP_ELVOC)

         !Organic condensate (from both elvoc and soa_sv) goes to the soaCondensateReceiver tracer (cka)
         ! smb++
         ! Should here instead call contend_sub_vbs and set intermed conc of SOA_A1 to that
         ! SOA_SV is zero.
         call cnst_get_ind ( 'SOA00', l_soa00, abort=.true.)
         !l_soa00 = chemistryIndex(ndx_dummy)
         q(i,k, chemistryIndex(l_soa00)) = q(i,k, chemistryIndex(l_soa00)) &
                 + gasLost(i,k,COND_VAP_ELVOC)*(1.0_r8-fracNucl(i,k,COND_VAP_ELVOC))  ! part of low volatile which does not nucleate
         !WRITE(*,*) q(i,k, chemistryIndex(l_soa00)), temperature
         call condtend_sub_vbs(q(i,k,:), temperature(i,k), pmid(i,k),q(i,k, chemistryIndex(l_soa00)), i,k )
         ! track SOA_LV condensate in it's own tracer.
         !q(i,k,chemistryIndex(l_soa_a1)) = q(i,k,chemistryIndex(l_soa_a1))         &
         !               + gasLost(i,k,COND_VAP_ORG_SV)                             &           ! "semi volatile" can not nucleate
         !               + gasLost(i,k,COND_VAP_ELVOC)*(1.0_r8-fracNucl(i,k,COND_VAP_ELVOC))  ! part of low volatile which does not nucleate
         !smb--
         !condenseable vapours
         !WRITE(*,*) chemistryIndex(l_h2so4), intermediateConcentration(i,k,COND_VAP_H2SO4)
         !WRITE(*,*) chemistryIndex(l_elvoc), intermediateConcentration(i,k,COND_VAP_H2SO4)

         q(i,k,chemistryIndex(l_h2so4))  = intermediateConcentration(i,k,COND_VAP_H2SO4)
         q(i,k,chemistryIndex(l_elvoc)) = intermediateConcentration(i,k,COND_VAP_ELVOC)
         !smb++ remove this line (covered in condtend_sub_vbs).
         !q(i,k,chemistryIndex(l_soa_sv)) = intermediateConcentration(i,k,COND_VAP_ORG_SV)
         !smb--

         ! smb++: might need to fix something with the OM_NI to OM_AI??
   ...
   subroutine condtend_sub_vbs( q, temperature, pmid,  SOA_LV_cond, i, k)!&
!           pmin, pdel, dt, ncol, pblh,zm,qh20)
      use constituents,    only: pcnst, cnst_name, cnst_get_ind
      use physconst,       only: avogad, rair
      use vbs_def,         only: oa_all_names, og_all_names
      use noresm_oracle,   only: oracle_soap, calc_loss
      implicit none
   ! arguments
      ...
      real(r8), intent(inout) :: q(gas_pcnst) ! TMR [kg/kg] including moisture

      real(r8), intent(inout) :: SOA_LV_cond !
      ...
      real (r8)   ::  kg2ug = 1e9
      integer,dimension(NSOAv) :: ndxs_all_oa
      integer,dimension(NSOAv) :: ndxs_all_og
      real(r8),dimension( NSOAP):: caer_vbs, cgas_vbs
      real(r8),dimension( NSOAP):: csatT
      ...
      do m=1,NSOAP
         spec_name = og_all_names(m)
         !WRITE(*,*) spec_name
         call cnst_get_ind ( trim(spec_name), ndx_dummy,abort=.true.)
         ndxs_all_og(m) = chemistryIndex(ndx_dummy)
         !WRITE(*,*) spec_name, ndx_dummy,ndxs_all_og(m),m

      end do
      do m=1,NSOAP
         spec_name = oa_all_names(m)

         call cnst_get_ind (  trim(spec_name), ndx_dummy,abort=.true.)
         ndxs_all_oa(m) = chemistryIndex(ndx_dummy)
         !WRITE(*,*) spec_name, ndx_dummy,ndxs_all_oa(m), m
      ...
      !do k=1,pver
      !   do i=1,ncol
      !rhoair(i,k)=pmid(i,k)/(temperature(i,k)*rair)/1000._r8 ! kg/m3
      rhoair=pmid/(temperature*rair)/1000._r8 ! kg/m3
      do m = 1,NSOAP

         tracerIndex = ndxs_all_og(m)
         !WRITE(*,*) 'tracerIndex', tracerIndex, gas_pcnst, NSOAP, m
         ! oracle_soap assumes data in ug/m3, need to convert from kg/kg
         !WRITE(*,*) 'q',q(i,k,tracerIndex)
         !WRITE(*,*) 'rho',rhoAir(i,k)
         !WRITE(*,*) 'kg2ug',kg2ug

         !cgas_vbs(i,k,m) =  q(i,k,tracerIndex)*rhoAir(i,k)*kg2ug ! kg_air/m3*ug_aer/kg_aer*kg_aer/kg_air
         cgas_vbs(m) =  q(tracerIndex)*rhoAir*kg2ug ! kg_air/m3*ug_aer/kg_aer*kg_aer/kg_air
         tracerIndex = ndxs_all_oa(m)
         !WRITE(*,*) 'q oa:', q(i,k,tracerIndex)
         caer_vbs(m) = q(tracerIndex)*rhoAir*kg2ug  ! kg_air/m3*ug_aer/kg_aer*kg_aer/kg_air
      end do
      call oracle_soap(caer_vbs(:),cgas_vbs(:),temperature,i,k,csatT)

      do m = 1,NSOAP
         tracerIndex = ndxs_all_og(m)
         ! oracle_soap assumes data in ug/m3, need to convert to kg/kg
         q(tracerIndex) = cgas_vbs(m)/(rhoAir*kg2ug)
         tracerIndex = ndxs_all_oa(m)
         ! oracle_soap assumes data in ug/m3, need to convert to kg/kg
         q(tracerIndex) = caer_vbs(m)/(rhoAir*kg2ug)
      end do

      ! set soa_a1 to be equal to the sum of all the condensed vapors
      ! also convert from ug/m3 to kg/kg
      q(chemistryIndex(l_soa_a1)) = sum((caer_vbs(:)/(rhoAir*kg2ug)))+SOA_LV_cond

      !   end do
      !end do
      !
   end subroutine condtend_sub_vbs
