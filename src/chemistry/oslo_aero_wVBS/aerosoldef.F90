...
!cka: add l_soa_n, l_soa_na (particles) l_soa_a1 (condensate) and l_elvoc, l_soa_sv (SOA precursors)
! following are species indices for individual camuio species
  ...
  l_soa_na, l_soa_a1, l_elvoc, l_soa_sv
...
!cka: register SOA species
    ...
    call cnst_get_ind('ELVOC',l_elvoc, abort=.true.) !Gas phase low volatile SOA
    ...
    rhopart(l_elvoc) = aerosol_type_density(AEROSOL_TYPE_OM)
